...
v
is.na(v)
wich(is.na(v))      # posizione di NA

sum(v)                  # NA
sum(v, na.rm = TRUE)    # ignora NA
sum(is.na(v))                # conta quanti NA ci sono
...
disturbo <- factor(c("basso", "alto", "medio", "basso"),
                   levels = c("basso", "medio", "alto"), #se non lo ordino io mette in ordine alfabetico 
                   ordered = TRUE) 
...
mat <- matrix(1:9, nrow = 3, ncol = 3)  #righe-osservazioni colonne-variabili
...
# Filtrare righe
df[df$quota > 1000, ] # solo siti con quota > 1000
...
### 10) LISTE ------------------------------------------------
#sono oggetti monodimensionali ma possono contenere oggetti di tipo diverso, anche altri data frame, matrici, tibbles, ecc.
